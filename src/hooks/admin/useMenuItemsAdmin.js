...
async function handleQuickCreateCategory(categoryName) {
  ...
  if (existedCategory) {
    ...
    setMessage(`Đã chọn danh mục "${existedCategory.name}".`);
  ...
  try {
    ...
    const categoryRef = await createCategory(DEFAULT_SHOP_ID, {
      name: cleanName,
      order: categories.length + 1,
      isActive: true,
    });
    ...
    setMessage(`Đã thêm và chọn danh mục "${cleanName}".`);

    return categoryRef.id;
  } catch (err) {
    console.error(err);
    setError("Không thể thêm nhanh danh mục.");
    return "";
  } finally {
    setCategorySubmitting(false);
  }
    ...
    try {
      ...
      if (filesToUpload.length > 0) {
        const uploadedImages = await uploadMediaFilesToCloudinary(
          filesToUpload,
          `home-coffee/${DEFAULT_SHOP_ID}/items`
        );
      ...
      const payload = {
        ...
        toppings: parseToppings(itemForm.toppingsText),
        sizes,
      };
  ...
  function handleEditItem(item) {
    ...
    setItemForm({
      ...
      toppingsText: formatToppingsText(item.toppings || []),
