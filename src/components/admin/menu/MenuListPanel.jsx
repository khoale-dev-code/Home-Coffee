...
export default function MenuListPanel({
  ...
  return (
    <section className="overflow-hidden rounded-[16px] border border-[#d9dda9] bg-white shadow-sm">
      <div className="border-b border-[#d9dda9] bg-white p-3 sm:p-4 lg:p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0">
            <p className="text-[11px] font-black uppercase tracking-[0.16em] text-[#294f31]">
              Product list
            </p>

            <h2 className="mt-1 text-lg font-black tracking-tight text-[#294f31] sm:text-2xl">
              Danh sách sản phẩm
            </h2>
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_220px_220px_120px]">
          <div className="relative min-w-0 md:col-span-2 xl:col-span-1">
            ...
            <input
              ...
              className="h-10 w-full rounded-[10px] border border-[#d9dda9] bg-white pl-10 pr-3 text-sm font-bold text-[#294f31] outline-none transition placeholder:text-neutral-400 focus:border-[#294f31] sm:h-11"
            />
          </div>
          ...
          <select
            ...
            className="h-10 rounded-[10px] border border-[#d9dda9] bg-white px-3 text-sm font-black text-[#294f31] outline-none transition focus:border-[#294f31] sm:h-11"
          >
          ...
          <div className="flex h-10 items-center justify-center rounded-[10px] border border-[#d9dda9] bg-[#f7f8ec] px-3 text-sm font-black text-neutral-600 sm:h-11">
            50/trang
          </div>
        </div>
...
function StatBox({ label, value }) {
  return (
    <div className="rounded-[10px] bg-[#f7f8ec] px-3 py-2 text-center ring-1 ring-[#d9dda9]">
      <p className="text-[10px] font-bold text-neutral-400 sm:text-[11px]">
        {label}
      </p>

      <p className="text-sm font-black text-[#294f31]">{value}</p>
    </div>
  );
}
  ...
  return (
    <article
      className={cn(
        ...
        isDragging
          ? "z-50 scale-[1.01] border-[#294f31] shadow-2xl"
          : "border-[#d9dda9] hover:border-neutral-300 hover:shadow-md"
      )}
    >
      <div className="relative h-[112px] w-[112px] shrink-0 bg-[#f7f8ec] min-[390px]:h-[124px] min-[390px]:w-[124px] sm:h-auto sm:w-full sm:aspect-square">
        {firstMedia ? (
        ...
        <div className="absolute right-1.5 top-1.5 flex gap-1 sm:right-2 sm:top-2 sm:gap-1.5">
          <IconButton
            ...
            title="Sửa sản phẩm"
            className="bg-white/95 text-[#294f31] hover:bg-[#294f31] hover:text-white"
          >
            <Edit size={14} />
          </IconButton>
          ...
          <IconButton
            ...
            className={
              isFeatured
                ? "bg-yellow-400 text-[#294f31] hover:bg-yellow-300"
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between gap-2 p-2.5 sm:block sm:space-y-2.5 sm:p-3">
        <div className="min-w-0">
          <p className="line-clamp-2 text-sm font-black leading-5 text-[#294f31] sm:min-h-[38px]">
        ...
        <div>
          <div className="flex flex-wrap items-baseline gap-1.5 sm:gap-2">
            <p className="text-sm font-black text-[#294f31] sm:text-base">
              {formatPrice(price)}
            </p>
        ...
        <div className="grid grid-cols-3 gap-1.5 border-t border-neutral-100 pt-2 sm:pt-2.5">
          <IconActionButton
            onClick={handleEditClick}
            title="Sửa sản phẩm"
            className="bg-[#294f31] text-white hover:bg-[#1f3d26]"
          >
            <Edit size={14} />
          </IconActionButton>
...
function PaginationBar({
  ...
  return (
    <div className="mt-4 flex flex-col gap-3 border-t border-[#d9dda9] pt-4 lg:flex-row lg:items-center lg:justify-between">
      ...
      <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-2">
        <button
          ...
          onClick={onPrev}
          disabled={page <= 1}
          className="inline-flex h-9 items-center justify-center gap-1 rounded-[8px] border border-[#d9dda9] bg-white px-3 text-xs font-black text-neutral-700 transition hover:bg-[#f7f8ec] disabled:cursor-not-allowed disabled:opacity-40 sm:h-10 sm:text-sm"
        >
          <ChevronLeft size={15} />
          Trước
        </button>
              ...
              <button
                ...
                className={cn(
                  ...
                  page === pageNumber
                    ...
                    : "border border-[#d9dda9] bg-white text-neutral-700 hover:bg-[#f7f8ec]"
                )}
              >
                {pageNumber}
              </button>
...
function EmptyState() {
  return (
    <div className="rounded-[14px] border border-dashed border-[#d9dda9] bg-[#f7f8ec] px-5 py-10 text-center sm:py-12">
  ...
  return (
    <div className="grid gap-2.5 sm:grid-cols-2 sm:gap-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
      {Array.from({ length: 10 }).map((_, index) => (
        <div
          key={index}
          className="flex overflow-hidden rounded-[13px] border border-[#d9dda9] bg-white shadow-sm sm:block sm:rounded-[14px]"
        >
