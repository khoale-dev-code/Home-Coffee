      ...
      */}
      <div className="relative z-10 flex h-[92dvh] w-full flex-col self-end overflow-hidden rounded-t-[24px] bg-[#FDFDFD] shadow-2xl animate-in slide-in-from-bottom-full duration-400 ease-out sm:h-[100dvh] sm:w-[480px] sm:rounded-none sm:slide-in-from-right-full">
        ...
        <div className="relative flex-1 overflow-y-auto px-5 py-6 sm:px-8 sm:py-8 [scrollbar-width:thin]">
          {/* Nhãn tag nổi */}
          {promotion.subtitle ? (
            <span className="inline-block rounded-full bg-neutral-100 px-3 py-1 text-[11px] font-black uppercase tracking-[0.14em] text-[#294f31]">
              {promotion.subtitle}
            </span>
          ) : (
            <span className="inline-block rounded-full bg-[#E8F0F2] px-3 py-1 text-[11px] font-black uppercase tracking-[0.14em] text-[#294f31]">
              Khuyến mãi
            </span>
          )}

          <h2 className="mt-4 text-2xl font-black leading-[1.3] tracking-tight text-[#294f31] sm:text-3xl">
            {promotion.title}
          </h2>
          ...
          {promotion.description && (
            <div className="mt-6 border-t border-neutral-100 pt-6">
              <p className="font-black text-[#294f31]">Nội dung ưu đãi</p>
          ...
          {promotion.terms && (
            <div className="mt-6 rounded-[12px] bg-neutral-50 p-4">
              <p className="font-black text-[#294f31]">Điều kiện áp dụng</p>
              <p className="mt-1.5 whitespace-pre-line text-[13px] leading-relaxed text-[#647343]">
                {promotion.terms}
              </p>
            </div>
          )}
        ...
        <div className="shrink-0 border-t border-neutral-100 bg-white p-4 pb-[max(env(safe-area-inset-bottom),1rem)] shadow-[0_-4px_20px_rgba(0,0,0,0.03)] sm:px-8">
          <button
            type="button"
            onClick={onClose}
            className="flex w-full items-center justify-center rounded-[12px] bg-[#294f31] px-5 py-4 text-[15px] font-black uppercase tracking-[0.12em] text-white transition-all hover:bg-[#4A372D] active:scale-[0.98]"
          >
            Tôi đã hiểu
          </button>
        </div>
...
function PromotionMediaFrame({ media, isActive = false }) {
  if (!media) {
    return (
      <div className="flex aspect-square sm:aspect-auto sm:h-[45vh] w-full flex-col items-center justify-center bg-neutral-100 text-[#819045]">
...
function InfoBadge({ label, value }) {
  return (
    <div className="flex items-center gap-2 rounded-full border border-neutral-200 bg-white px-3.5 py-1.5 shadow-sm">
      <CalendarDays size={14} className="text-[#819045]" />
      <p className="text-[12px] font-medium text-[#294f31]">
        <span className="font-bold mr-1 text-[#819045]">{label}:</span> 
        {value}
      </p>
    </div>
  );
