  ...
  return (
    ...
    >
      ...
      <div className="mb-6 flex flex-col gap-2 border-b border-neutral-100 pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-[#819045]">
            Khuyến mãi
          </p>
          <h2 className="mt-1.5 text-2xl font-black tracking-tight text-[#294f31] sm:text-3xl">
            Ưu đãi đang diễn ra
          </h2>
          <p className="mt-2 max-w-xl text-[15px] font-medium leading-relaxed text-[#647343]">
      ...
      */}
      <div className="relative aspect-[4/3] w-full shrink-0 overflow-hidden border-b border-neutral-100 bg-neutral-50/80 sm:aspect-[16/9]">
        {hasVideo ? (
          <>
            ...
            <div className="absolute inset-0 grid place-items-center bg-black/5 transition-colors group-hover:bg-transparent">
              <div className="grid h-12 w-12 place-items-center rounded-full bg-white/90 text-[#294f31] shadow-md backdrop-blur-sm transition-transform group-hover:scale-110">
          ...
          <div className="grid h-full w-full place-items-center bg-neutral-50 text-[#819045]">
        ...
        <div className="absolute left-3 top-3 flex flex-wrap gap-1.5 sm:left-4 sm:top-4">
          <span className="rounded-[6px] bg-[#294f31] px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.08em] text-white shadow-sm">
            Ưu đãi
          </span>
        </div>

        {mediaCount > 1 && (
          <span className="absolute bottom-3 right-3 rounded-[6px] bg-white/95 px-2.5 py-1 text-[10px] font-black text-[#294f31] shadow-sm backdrop-blur-md ring-1 ring-neutral-200/50">
            {mediaCount} media
          </span>
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col p-4 sm:p-5 sm:pb-6">
        {promotion.subtitle && (
          <p className="line-clamp-1 text-[11px] font-black uppercase tracking-[0.14em] text-[#819045]">
            {promotion.subtitle}
          </p>
        )}

        <h3 className="mt-1.5 line-clamp-2 text-[17px] font-black leading-snug text-[#294f31] sm:text-xl">
          {promotion.title}
        </h3>

        {promotion.description && (
          <p className="mt-2 line-clamp-2 text-[13px] font-medium leading-relaxed text-[#647343] sm:text-sm">
            {promotion.description}
          </p>
        )}
        ...
        <div className="mt-auto pt-5">
          <div className="flex items-center justify-between gap-3">
            <span className="hidden text-[11px] font-black uppercase tracking-[0.12em] text-[#A3A3A3] transition-colors group-hover:text-[#819045] min-[430px]:inline">
              Bấm để xem chi tiết
            </span>

            {/* Micro-interaction: Mũi tên dịch chuyển nhẹ khi hover card */}
            <span className="inline-flex w-full items-center justify-center gap-2 rounded-[8px] border border-[#294f31] bg-white px-4 py-2.5 text-[11px] font-black uppercase tracking-[0.08em] text-[#294f31] shadow-sm transition-all duration-300 group-hover:bg-[#294f31] group-hover:text-white group-active:scale-95 min-[430px]:w-auto sm:text-xs">
